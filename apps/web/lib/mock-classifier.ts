/**
 * INLINED COPY OF @counter/classifier createMockClassifier()
 *
 ...
 *     createClassifier,
 *     type Classifier,
 *   } from "@counter/classifier";
 *
 * Hand-tuned to land on the demo numbers from docs/DEMO_SCRIPT.md:
 ...
 *
 * If you change the table below, the runtime assertion at the bottom will fail.
 * That's intentional — the demo script copy depends on $892.
 */
...
function classifyOne(candidate: DisputeCandidate): ClassifiedDispute {
...
export function createClassifier(): Classifier {
  throw new Error(
    "createClassifier (real Claude) is owned by Worker 2 (@counter/classifier). " +
      "Use createMockClassifier() until that ships."
  );
}
